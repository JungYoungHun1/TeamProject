...
//오늘의 음식 이미지 가져오기
const refoodImgArr = document.querySelectorAll('.rdfood-img');
const setRdFood = ()=>{
    refoodImgArr.forEach(item=>{
        fetch(`/food/random`,{
            'method': 'post',
            ...
            .then((data)=>{
                console.log(data);
                let infoElem = item.querySelector('.rdfood-info');
                infoElem.innerHTML=`
               [${data.f_worlddiv}] [${data.f_cookery}] ${data.f_nm}
            `;
                getFoodImgMain(data.f_nm,item);
            }).catch(err=>{
            console.log(err);
        });
    });
...
// 음식 랜덤으로 ( 계절/술 코드 보내서 )
const getSAFood = (code,subCode,showFc) =>{
    fetch(`/ajax/common/${subCode}?code=${code}`).then(res=>res.json()).then(data=>{
        console.log(data);
        data.forEach(item=>{
...
getWeather((data)=>{
    ...
    getSAFood(1,seasonCode,makeImg);
});
/*
* 계절,술 콜백정리
...
alkElems.forEach(item=>{
    item.addEventListener('click',e=>{
        let acListElem = document.querySelector('#ac_list');
        while (acListElem.hasChildNodes()){
            acListElem.firstChild.remove();
        }
        ...
        getSAFood(2,item.value,makeImg);
    });
});

//첫 로딩때는 띄어주기
getSAFood(2,1,makeImg);
